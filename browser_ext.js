 ...
 const styles = () => {
    return ` <style> @import url("https://fonts.googleapis.com/css?family=Share+Tech+Mono|Montserrat:700");
...
function _ini(){
    ...
    window.onload=function(){
        var words = ["xxx", "sex","porn","videos","pornstar","pussy","free","porno","puba","pornhub","brazzers","xvideos"];
        var listWords = document.getElementsByTagName("title")[0].textContent.toLowerCase().split(/[ ,.]+/);
        aduleWordList= getMeta('keywords').toLowerCase().split(/[ ,.]+/);
        if( words.filter(value => listWords.includes(value)).length>1 || words.filter(value => aduleWordList.includes(value)).length>3){
